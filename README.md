...
1. Slack App管理画面の「OAuth & Permissions」で、Bot Token Scopesに `commands` を追加します。
2. Slack App管理画面で「Slash Commands」を開きます。
3. 「Create New Command」を押します。
4. 以下を設定します。
...
5. Slack App管理画面の「Basic Information」→「App Credentials」からVerification Tokenを確認します。
6. GASのScript Propertiesに `SLACK_VERIFICATION_TOKEN` として登録します。
7. Scopeを追加した場合は、Slack Appをワークスペースへ再インストールします。
